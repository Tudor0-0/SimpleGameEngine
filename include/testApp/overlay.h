...
#define TEXTURE_BUTTON(buttonName, buttonPoz, buttonState)                                                             \
    Texture buttonName = {{48 * buttonState, 32 * buttonPoz, 48, 32}, TextureSheets::wControlButtons}
...
public:
    ...
    void OnEvent(const Event &p_event) override {
        ...
        dispatcher.Dispatch<MouseButtonPressedEvent>([this](const MouseButtonPressedEvent &event) -> bool {
            if (event.GetButtonCode() == SDL_BUTTON_LEFT) {
                if (m_close.OnMousePressed())
                    return true;
                if (m_minimize.OnMousePressed())
                    return true;
                if (m_restore.OnMousePressed())
                    return true;
            }
            return false;
        });
        dispatcher.Dispatch<MouseButtonReleasedEvent>([this](const MouseButtonReleasedEvent &event) -> bool {
            if (event.GetButtonCode() == SDL_BUTTON_LEFT) {
                if (m_close.OnMouseReleased())
                    return true;
                if (m_minimize.OnMouseReleased())
                    return true;
                if (m_restore.OnMouseReleased())
                    return true;
            }
            return false;
        });
    ...
    Overlay() {
        m_close.Init(
            {1600 - 48, 0, 48, 32}, {closeNormal, closeHovered, closePressed}, [this]() { m_close.SetCurrentFrame(1); },
            [this]() { m_close.SetCurrentFrame(0); }, [this]() { m_close.SetCurrentFrame(2); },
            [this]() {
                m_close.SetCurrentFrame(1);
                m_core->Stop();
            },
            [this]() { m_close.SetCurrentFrame(1); });
        m_restore.Init(
            {1600 - 48 * 2, 0, 48, 32}, {restoreNormal, restoreHovered, restorePressed},
            [this]() { m_restore.SetCurrentFrame(1); }, [this]() { m_restore.SetCurrentFrame(0); },
        ...
        m_minimize.Init(
            {1600 - 48 * 3, 0, 48, 32}, {minimizeNormal, minimizeHovered, minimizePressed},
            [this]() { m_minimize.SetCurrentFrame(1); }, [this]() { m_minimize.SetCurrentFrame(0); },
