...
public:
    ...
    template <typename T> std::vector<std::unique_ptr<Layer>>::iterator GetLayerIterator() {
    ...
    template <typename T> void EraseLayer() { EraseLayer(GetLayer<T>()); }

    template <typename T> void FocusLayer() {
        LayerCommand command;
