...
struct WindowSettings {
    ...
    WindowSettings(const WindowSettings &p_windowSettings)
        : m_title(p_windowSettings.m_title), posx(p_windowSettings.posx), posy(p_windowSettings.posy),
          width(p_windowSettings.width), height(p_windowSettings.height), targetFps(p_windowSettings.targetFps),
          windowFlags(p_windowSettings.windowFlags), rendererFlags(p_windowSettings.rendererFlags) {
    ...
    WindowSettings(WindowSettings &&p_windowSettings) noexcept
        : m_title(std::move(p_windowSettings.m_title)), posx(p_windowSettings.posx), posy(p_windowSettings.posy),
          width(p_windowSettings.width), height(p_windowSettings.height), targetFps(p_windowSettings.targetFps),
          windowFlags(p_windowSettings.windowFlags), rendererFlags(p_windowSettings.rendererFlags) {
